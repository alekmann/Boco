...
import { getTheirRentals } from "@/service/RentalService.js";
...
export default {
  ...
  async created() {
    ...
    const response = await getTheirRentals(org);

    let rentals = response.data;
    if (org) {
      rentals = rentals.filter((rental) => rental.post.organization);
      rentals = rentals.filter(
        (rental) => rental.post.organization?.orgNum === org.orgNum
      );
    } else {
      rentals = rentals.filter((rental) => rental.post.organization === null);
    }
    ...
    for (let i = 0; i < rentals.length; i++) {
      ...
      if (rentalDate < newdate) {
        ...
        if (response.data[i].approved) this.rentalsApproved.push(rentals[i]);
        if (!response.data[i].approved)
          this.rentalsNotApproved.push(rentals[i]);
      }
    }
    this.rentalsApprovedCollecting = false;
    this.rentalsNotApprovedCollecting = false;
